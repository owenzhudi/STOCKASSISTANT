<?php
	...
	class query
	{
		...
		{
			return "SELECT DISTINCT s.Ticker, s.Price, p.LowestStockYear FROM Stocks s, Predictions p WHERE s.StockID = p.StockID ORDER BY Date desc LIMIT 20";
			//return "SELECT s.Ticker, s.Price, p.LowestStockYear, MAX(p.Date) FROM stocks s, predictions p WHERE s.StockID = p.StockID GROUP BY s.Ticker";
		}

		//
		public function getLowerThanGoogle() {
			return "SELECT s.StockID, s.Company, p.AvgYear, p.Date FROM Stocks s, Predictions p WHERE s.StockID = p.StockID AND p.AvgYear < 
             (SELECT LowestStockYear FROM predictions WHERE StockID=\"22\" AND Date>'2016-01-01 00:00:00') ORDER BY p.Date DESC";
		}
